...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar container */}
      <aside
        className={`fixed md:sticky top-0 left-0 z-40 h-screen w-72 flex flex-col bg-slate-950 text-slate-200 border-r border-slate-900 transition-transform duration-300 ease-in-out shadow-2xl md:translate-x-0 ${
      ...
      >
        ...
        {/* Sidebar Navigation */}
        <nav className="flex-1 overflow-y-auto px-4 py-6 space-y-1.5 scrollbar-thin scrollbar-thumb-slate-900">
          {links.map((item) => {
        ...
        {user && (
          <div className="p-4 border-t border-slate-900 bg-slate-950/40">
            <div className="flex items-center justify-between gap-3 p-3 rounded-2xl bg-slate-900/50 border border-slate-900/80 shadow-lg">
              <div className="flex items-center gap-3 min-w-0">
                <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-500 text-white font-bold text-sm flex items-center justify-center shadow-md shrink-0">
